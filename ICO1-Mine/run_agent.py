...
def main():
    ...
    nnet_args = {}
    nnet_args['n_hidden'] = np.array([80, 80])
    nnet_args['imgs_shape'] = [agent_args['history_length']*simulator.num_channels, simulator.resolution[1], simulator.resolution[0]]
    nnet_args['n_actions'] = 6
    nnet_args['n_game_vars'] = simulator.num_meas

#    visPredictor = Nnet(sess, nnet_args)
#    visPredictor.make_ffnet()
...
#            curr_act[4] = 0
            ...
            if (not (meas is None)) and meas[0] > 30.:
                meas[0] = 30.
#            if (not (img is None)):
#                visPredictor.act_ffnet(np.ndarray.flatten(state_imgs), np.ndarray.flatten(state_meas),
#                             np.array(curr_act[:6], dtype='float64'), np.ndarray.flatten(ag.preprocess_input_images(img)))
