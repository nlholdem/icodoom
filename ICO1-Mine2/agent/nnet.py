...
class Nnet:
    ...
    def make_ffnet(self):
        with self.sess.as_default():
            ...
            ffnet_W_layer1 = self.weight_variable([self.n_inputs, self.n_hidden[0]])
            ffnet_b_layer1 = self.bias_variable([self.n_hidden[0]])

            ffnet_W_layer2 = self.weight_variable([self.n_hidden[0], self.n_hidden[1]])
            ffnet_b_layer2 = self.bias_variable([self.n_hidden[1]])

            ffnet_W_layer3 = self.weight_variable([self.n_hidden[1], self.n_outputs])
            ffnet_b_layer3 = self.bias_variable([self.n_outputs])

            ffnet_h_1 = tf.nn.relu(tf.matmul(self.ffnet_input, ffnet_W_layer1) + ffnet_b_layer1)
            ffnet_h_2 = tf.nn.relu(tf.matmul(ffnet_h_1, ffnet_W_layer2) + ffnet_b_layer2)

            # dropout
            self.keep_prob = tf.placeholder(tf.float32)
            my_drop = tf.nn.dropout(ffnet_h_2, self.keep_prob)
            print("output shape: ", self.ffnet_output.get_shape(), "target shape: ", self.ffnet_target.get_shape())
            print("W3: ", ffnet_W_layer3.get_shape(), " bias3: ", ffnet_b_layer3.get_shape())

            self.ffnet_output = tf.matmul(ffnet_h_2, ffnet_W_layer3) + ffnet_b_layer3

            self.loss = tf.squared_difference(self.ffnet_output, self.ffnet_target)

            self.ffnet_train_step = tf.train.GradientDescentOptimizer(1e-4).minimize(self.loss)

            self.accuracy = tf.reduce_mean(self.loss)

            print ("* make_ffnet * vars to initialise: ", set(tf.all_variables()) - temp)

            self.sess.run(tf.initialize_variables(set(tf.all_variables()) - temp))
    #        self.init_op = tf.initialize_variables([W_layer1, b_layer1, W_layer2, b_layer2, W_layer3, b_layer3])
