...
public:
	...
	inline double getWeightChange( int _index,  int _filter = 0) {
		assert((_index>=0)&&(_index<nInputs)&&(_filter>=0)&&(_filter<nFilters));
		return mask[_index] ? weightChange[_index][_filter] : 0;
	};

	inline double getFilteredInput( int _index,  int _filter = 0) {
		assert((_index>=0)&&(_index<nInputs)&&(_filter>=0)&&(_filter<nFilters));
		return mask[_index] ? filteredInputs[_index][_filter] : 0;
	};
